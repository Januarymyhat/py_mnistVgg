...
class MainWidget(QWidget):
    ...
    def __init__(self):
        ...
        # 添加一系列控件
        self.paintBoard = paintBoard.PaintBoard(self)
        # 上传文件显示的board
        ...
        self.__comboBox_penColor.currentIndexChanged.connect(lambda: self.on_PenColorChange())  # 关联下拉列表的当前索引变更信号与函数on_PenColorChange
    ...
    def btn_file_on_click(self):
        ...
        if fiDir == '':
            ...
            image = QPixmap(fiDir).scaled(800, 800)
    ...
    def btn_clear_on_clicked(self):
        ...
        if self.paintBoard.isEmpty:
            self.label_showBoard.setPixmap(QPixmap(""))
            return self.label_showBoard.hide()
        else:
            return self.paintBoard.Clear()